...
class ParallelForecastDecoder(nn.Module):
    def __init__(
        self,
        pred_len: int,
        d_model: int,
        ...
        # Query aprendida para cada passo futuro
        self.horizon_embedding = nn.Parameter(
            torch.zeros(1, pred_len, d_model)
        )

        # Embedding do canal/ativo
        self.channel_embedding = nn.Embedding(max_channels, d_model)

        # Condiciona o decoder no último valor observado normalizado
        self.last_value_embedding = nn.Linear(1, d_model)

        # Atenção entre tokens futuros: todos os H passos são previstos juntos
        ...
        # Cross-attention: queries futuras consultam a memória do encoder
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=d_model,
            num_heads=n_heads,
            dropout=dropout,
    ...
    def forward(self, memory: torch.Tensor, x_norm: torch.Tensor):
        """
        ...
        """
        if memory.ndim != 4:
            raise ValueError(f"memory esperado (B,N,T,D), veio {memory.shape}")
        if x_norm.ndim != 3:
            raise ValueError(f"x_norm esperado (B,T,N), veio {x_norm.shape}")
        ...
        # memória do encoder: (B, N*T, D)
        mem = memory.reshape(B, N * T, D)

        # embeddings de horizonte: (1, H, 1, D)
        h_emb = self.horizon_embedding.to(device=device, dtype=dtype)
        h_emb = h_emb[:, :, None, :]

        # embeddings de canal: (1, 1, N, D)
        ch_idx = torch.arange(N, device=device)
        ch_emb = self.channel_embedding(ch_idx).to(dtype=dtype)
        ch_emb = ch_emb[None, None, :, :]

        # último valor observado: (B, 1, N, D)
        last_x = x_norm[:, -1, :].unsqueeze(-1)          # (B, N, 1)
        last_emb = self.last_value_embedding(last_x)     # (B, N, D)
        last_emb = last_emb[:, None, :, :]               # (B, 1, N, D)

        # queries futuras: (B, H, N, D)
        q = h_emb + ch_emb + last_emb

        # sequência de tokens futuros: (B, H*N, D)
        q = q.reshape(B, H * N, D)

        # self-attention paralela no bloco futuro inteiro
        self_out, _ = self.self_attn(q, q, q, need_weights=False)
        q = self.norm1(q + self_out)

        # cross-attention contra encoder
        cross_out, _ = self.cross_attn(q, mem, mem, need_weights=False)
        q = self.norm2(q + cross_out)

        q = self.norm3(q + self.ffn(q))

        y = self.out_proj(q).squeeze(-1)   # (B, H*N)
        y = y.reshape(B, H, N)
...
class AttentionForecastModel(nn.Module):
    def __init__(self, configs):
        ...
        self.decoder = ParallelForecastDecoder(
            pred_len=self.pred_len,
            d_model=self.d_model,
            n_heads=self.n_heads,
            ff_dim=self.ff_dim,
...
class CrossAttentionAdapterChannelEncDecPar(DeepForecastingModelBase):
