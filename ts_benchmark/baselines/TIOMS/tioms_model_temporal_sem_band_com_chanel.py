...
class ExpertAttention(nn.Module):
    def __init__(
        ...
        # cabeça final depende do tipo de agregação temporal
    ...
    def forward(self, x: torch.Tensor):
        ...
        # FFN
        ffn_out = self.ffn(z)
        z = self.norm2(z + ffn_out)                       # (B*N, T, d_model)

        # agregação temporal
        z_summary = self._pool_time(z)

        # projeção para horizonte
        y = self.head(z_summary)                          # (B*N, pred_len)
...
class NoBandWiseForecastModel(nn.Module):
    def __init__(self, configs):
        ...
        self.temporal_pool_type = getattr(configs, "temporal_pool_type", "flat")
