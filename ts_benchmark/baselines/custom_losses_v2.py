"""Losses financeiras cross-section v2 para o experimento B3.

A implementação é opt-in: ``scripts/run_benchmark_custom_losses_v2.py``
substitui a fábrica de losses apenas durante a execução do experimento v2.
O módulo v1 permanece intacto e continua disponível.
"""
...
def build_loss(config, normalizer_mean=None, normalizer_scale=None):
    """Constrói uma loss v2; nomes não-v2 são delegados à implementação v1."""
...
class FinancialScoreAggregatorV2(nn.Module):
    ...
    def _maybe_inverse_norm(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scale = self.normalizer_scale[..., :n_series].to(device=x.device, dtype=x.dtype)
        if x.dim() == 2:
    ...
    def _select_horizon(
        ...
        if self.horizon_mode == "strict":
            if horizon != self.loss_k:
                raise ValueError(
                    "Inconsistência H/K: modo strict exige H == loss_k; "
                    f"recebido H={horizon}, loss_k={self.loss_k}."
                )
            return pred, target
        if horizon < self.loss_k:
            raise ValueError(f"H={horizon} é menor que loss_k={self.loss_k}.")
    ...
    def forward(
        ...
        if self.data_kind in {"price", "prices", "preco", "preços"}:
            if base_value is None:
                raise ValueError(
                    "loss_data_kind='price' exige o último preço observado em base_value."
                )
...
class FinancialObjectiveV2(nn.Module):
    """Objetivos financeiros v2 com H/K e escalas explicitamente controlados."""
    ...
    def _ranknet(self, pred_score: torch.Tensor, target_score: torch.Tensor) -> torch.Tensor:
        pred_score, target_score = self._normalized_scores(pred_score, target_score)
        pred_diff, target_diff, valid = self._upper_pairwise(pred_score, target_score)
        sign = torch.sign(target_diff)
        values = F.softplus(-self.ranknet_alpha * sign * pred_diff)
        return self._masked_mean(values, valid)

    def _listnet(self, pred_score: torch.Tensor, target_score: torch.Tensor) -> torch.Tensor:
        pred_score, target_score = self._normalized_scores(pred_score, target_score)
    ...
    @staticmethod
    def _score_mse(pred_score: torch.Tensor, target_score: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        if self.loss_name == "mse_score_v2":
            total = self._score_mse(pred_score, target_score)
            self._remember(score_mse=total, total=total)
            return total

        if self.loss_name == "ranknet_v2":
            total = self._ranknet(pred_score, target_score)
            self._remember(rank=total, total=total)
            return total
        ...
        if self.loss_name == "listnet_v2":
            total = self._listnet(pred_score, target_score)
            self._remember(listnet=total, total=total)
            return total
