#!/usr/bin/env python3
"""Testes determinísticos das losses financeiras v2."""
...
def test_log_return_aggregation():
    ...
    assert_close(criterion(target, target), torch.tensor(0.0))
...
def test_path_mse_matches_torch():
    torch.manual_seed(7)
    pred = torch.randn(2, 3, 4)
    target = torch.randn(2, 3, 4)
    criterion = build_loss(config("mse_path_v2"))
    assert_close(criterion(pred, target), torch.nn.functional.mse_loss(pred, target))
