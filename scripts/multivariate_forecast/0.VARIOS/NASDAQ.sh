...
SALVAR_EM="NASDAQ/seqlen${SEQ_LEN}/h${PRED_LEN}"

echo "DUET"
$PYTHON_BIN  ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json" --data-name-list "NASDAQ.csv" --strategy-args '{"horizon": '$PRED_LEN'}' --model-name "duet.DUET" --model-hyper-params '{"CI": 1, "batch_size": 8, "d_ff": 1024, "d_model": 512, "dropout": 0.2, "e_layers": 2, "factor": 3, "pred_len": '$PRED_LEN', "k": 2, "loss": "MAE", "lr": 0.0005, "lradj": "type1", "n_heads": 1, "norm": true, "num_epochs": 100, "num_experts": 4, "patch_len": 48, "patience": 5, "seq_len": '$SEQ_LEN'}' --deterministic "full" --gpus 0 --num-workers 1 --timeout 60000 --save-path "experimentos/$SALVAR_EM" --save-true-pred False

echo "TIMESNET"
$PYTHON_BIN  ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json" --data-name-list "NASDAQ.csv" --strategy-args '{"horizon": '$PRED_LEN'}' --model-name "time_series_library.TimesNet" --model-hyper-params '{"d_ff": 512, "d_model": 256, "factor": 3, "pred_len": '$PRED_LEN', "norm": true, "seq_len": '$SEQ_LEN', "top_k": 5}' --adapter "transformer_adapter" --gpus 0 --num-workers 1 --timeout 60000 --save-path "experimentos/$SALVAR_EM" --save-true-pred False

echo "TIOMS LINEAR"
$PYTHON_BIN ./scripts/run_benchmark.py --config-path "rolling_forecast_config.json" --data-name-list "NASDAQ.csv" --strategy-args '{"horizon": '$PRED_LEN'}' --model-name "TIOMS.LearnableBandWiseAdapterAudit" \
